...
def main():
    ...
    # ----------- 초기 집게 벌리기 -----------
    # 시작 시 집게만 벌리고 대기 (reset()에서 이미 GRIP_OPEN으로 설정됨)
    time.sleep(1)  # 잠깐 대기 (초기화 완료될 때까지)
    ...
    try:
        while True:
            ...
            if state == "SEARCH":
                ...
                # 개선된 집기 함수 사용 - 내리기만 (들어올리기는 LIFT에서)
                arm.grab_once_improved_lower_only(grip_close=GRIP_CLOSE)
                state = "LIFT"

            elif state == "LIFT":
                last_seen_time = time.time()
                arm.lift_improved()
                print("[DEBUG] LIFT 완료 → 안정화 대기")

                # 추가 안정화 시간 (물건이 떨어지지 않도록)
                time.sleep(0.5)
            ...
            # 시야 상실 타임아웃 (LIFT 제외)
            # ---------------------------------------------------
            if state in ("LOWER") and (time.time() - last_seen_time) > LOSE_SIGHT_TIMEOUT:
                arm.recover_soft()
                state = "LOCK"
                pos_stable = 0
